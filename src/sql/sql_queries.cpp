...
int generate_bit_order(QString table, int array[], QString filter_column) {
    QString stmt = QString("SELECT id FROM %1 %2").arg(table).arg((filter_column == "" ? QString("") :
                   QString("WHERE %1=1").arg(filter_column)));
    perform_query(stmt);
    int i = 0;
    bool ok;
    ...
    while (theSarf->query.next()) {
        ...
        if (ok == false) {
            _error << "Unexpected Error: Non-integer ID\n";
...
int append_to_bit_order(int array[], int id) {
    if ((++array[max_sources]) > max_sources) {
        _error << "Unexpected Error: Number of elements in the array has passed the limit!\n";
...
int get_bitindex(int id, int array[]) { //very slow, implement differently later
    for (int i = 0; i < max_sources &&
         i < array[max_sources]; i++) //array[max_sources] stores by convention the number of filled entries in the array
        if (array[i] == id) {
            return i;
        }

    _error << "Unexpected Error: id is not part of the ids array\n";
...
QString bitset_to_string(dbitvec b) {
    ...
    QChar *val = new QChar[num_characters];
    ...
    QString ret_val(val, num_characters);
    delete[] val;
    return ret_val;
}
...
long long getID(QString table, QString name, QString additional_condition, QString column_name) {
    ...
    stmt = stmt.arg(table).arg(name).arg((additional_condition == QString("") ? additional_condition : "AND " +
                                          additional_condition)).arg(column_name);
    perform_query(stmt);
...
QString getColumn(QString table, QString column_name, long long id, QString additional_condition, bool has_id) {
    ...
    stmt = stmt.arg(table).arg((has_id ? QString("id ='%1'").arg(id) : "")).arg((additional_condition == QString("") ?
            additional_condition : (has_id == true ? "AND " : "") + additional_condition)).arg(column_name);
...
bool existsSOURCE(int source_id) {
    if (source_id < 0 || source_id >= max_sources || !existsID("source", source_id)) {
        _error << "INVALID source_id! Unable to perform operation...\n";
        return false;
    } else {
        return true;
    }
...
bool existsEntry(QString table, unsigned long long id = -1, QString additional_condition = "", bool has_id = true) {
    ...
    stmt = stmt.arg(table).arg((has_id ? QString("id ='%1'").arg(id) : QString(""))).arg((
                additional_condition == "" ? additional_condition : (has_id ? "AND " : "") + additional_condition));
...
dbitvec get_bitset_column(QString table, QString column, unsigned long long id = -1, QString additional_condition = "",
                          bool has_id = true) {
    QString stmt("SELECT %4 FROM %1 WHERE %2 %3");
    stmt = stmt.arg(table).arg((has_id ? QString("id ='%1'").arg(id) : QString(""))).arg((
                additional_condition == "" ? additional_condition : (has_id ? "AND " : "") + additional_condition)).arg(column);
...
int get_type_of_category(long category_id, item_types &type) {
    ...
    if (theSarf->query.next()) {
        ...
        if (!ok) {
            _error << "Unexpected Error: Non-integer type\n";
            return -2;
        }
...
dbitvec set_index_bitset(QString table, QString column_name, int index, long long id = -1 ,
    ...
    stmt.append(str1);
    stmt.append("' WHERE ").append((has_id == true ? QString("id ='%1'").arg(id) : QString(""))).append((
                additional_condition == "" ? additional_condition : (has_id ? " AND " : "") + additional_condition));
    QString where;
    where.append("' WHERE ").append((has_id == true ? QString("id ='%1'").arg(id) : QString(""))).append((
                additional_condition == "" ? additional_condition : (has_id ? " AND " : "") + additional_condition));
...
dbitvec addSource(QString table, int source_id, long long id , QString additional_condition, bool has_id) {
    ...
    if (bit_index >= 0 && bit_index < max_sources) {
        ...
        _error << "Unexpected Error: source_id =" << source_id << "\n";
        return INVALID_BITSET;
    }
}
dbitvec addAbstractCategory(QString primary_condition, int abstract_category_id) {
    ...
    if (bit_index >= 0 && bit_index < max_sources) {
        ...
        _error << "Unexpected Error: abstract_category_id =" << abstract_category_id << "\n";
        return INVALID_BITSET;
    }
}
bool addAbstractCategory(QString name, QString raw_data, QString category, int source_id, QList<long> *abstract_ids,
    ...
    if (abstract_ids != NULL) {
        for (int i = 0; i < abstract_ids->count(); i++)
            if (abstract_ids->operator [](i) == -1 ||
                !existsID("category", abstract_ids->operator [](i), QString("abstract=1 AND type =%1").arg((int)(STEM)))) {
                if (abstract_ids->operator [](i) != -1) {
    ...
    if (category_id == -1) {
        ...
        if (cat_sources == INVALID_BITSET) {
            _error << "Unexpected Error: Category must exist but this seems not the case\n";
            return false;
        }
    }
    ...
    if (item_id == -1) {
        _error << "Item does not exist\n";
        return false;
    } else if (abstract_ids != NULL) {
        for (int i = 0; i < abstract_ids->count(); i++) {
            int bit_index = get_bitindex(abstract_ids->operator [](i), abstract_category_ids);

            if (!(bit_index >= 0 && bit_index < max_sources)) {
                _error << "Unexpected Error: abstract_id =" << abstract_ids->operator [](i) << "\n";
                return false;
            }
        ...
        _error << "No categories provided\n";
        return false;
    }

    QString primary_condition =
        QString("%1_id = '%2' AND category_id = '%3' AND raw_data='%4' AND description_id = %5 AND POS=\"%6\"").arg(table).arg(
            item_id).arg(category_id).arg(raw_data).arg(description_id).arg(POS);
...
int resolve_conflict(QString table, QString column_name, QVariant new_value, QString primary_key_condition,
                     int source_id, bool nonvague_sources = true/*, QString additional_SET_condition=""*/) {
    ...
    */
    ...
    if (theSarf->query.size() == 0) {
        _error << "Unexpected Error: No conflict is present since row does not even exist\n";
        return -2;
    }
    ...
    if (old_value == new_value) {
        addSource(table, source_id, -1, primary_key_condition,
                  false);    //just to be generic we assume no column id exist, but even if it is no problem condition can solve this
    } else {
        if ((KEEP_OLD && !isNull) || (!KEEP_OLD && new_value.isNull())) {
            _warning <<
                     QString("CONFLICT with '%1' in table '%2' at entry satisfying the following condition (%3). KEPT %1 %4 instead of %5\n").arg(
                         column_name).arg(table).arg(primary_key_condition).arg(old_value.toString()).arg(new_value.toString());
            ...
            stmt = QString("UPDATE %1 SET %2='%3' %4 WHERE %5").arg(table).arg(column_name).arg(new_value.toString()).arg(
                       additional_SET_condition).arg(primary_key_condition);
            //qDebug() << stmt;
            perform_query(stmt);

            if (!isNull) {
                _warning <<
                         QString("CONFLICT with '%1' in table '%2' at entry satisfying the following condition (%3). Replaced %1 %5 instead of %4\n").arg(
                             column_name).arg(table).arg(primary_key_condition).arg(old_value.toString()).arg(new_value.toString());
            } else {
                _warning <<
                         QString("UPDATED '%1' in table '%2' at entry satisfying the following condition (%3), from NULL to %1 = '%4'\n").arg(
                             column_name).arg(table).arg(primary_key_condition).arg(new_value.toString());
            }
        }
    }
...
long insert_category(QString name, item_types type, dbitvec sources,
                     bool isAbstract) { //returns its id if already present and if names are equal but others are not, -1 is returned
    ...
    if (isAbstract)
        if (append_to_bit_order(abstract_category_ids, id) < 0) {
            _error << "Unexpected Error: Error generating abstract category array\n";
...
long insert_category(QString name, item_types type, int source_id,
                     bool isAbstract) { //returns its id if already present
    dbitvec sources(max_sources);
    sources.reset();
    int bit_index = get_bitindex(source_id, source_ids);
...
long insert_item(item_types type, QString name, QString raw_data, QString category, int source_id,
    ...
    if (table == "--") {
        _error << "Table Invalid\n";
        return -3; //must not reach here
    }

    if (!existsSOURCE(source_id)) {
        _error << "Source does not exist\n";
        return -2;
    }
    ...
    if (abstract_ids != NULL) {
        for (int i = 0; i < abstract_ids->count(); i++)
            if (abstract_ids->operator [](i) == -1 ||
                !existsID("category", abstract_ids->operator [](i), QString("abstract=1 AND type =%1").arg((int)(type)))) {
                if (abstract_ids->operator [](i) != -1) {
                    _warning << QString("Undefined Abstract Category Provided '%1'. Will be ignored\n").arg(abstract_ids->at(i));
                }
    ...
    if (category_id == -1) {
        ...
        if (category_id == -1) {
            _error << "Category was not inserted sucessfully\n";
            return -1;
        }
    ...
    if (item_id == -1) {
        if (type == STEM) {
            ...
            stmt = stmt.arg(name).arg((grammar_stem_id == -1 ? QString("NULL") : QString("'%1'").arg(grammar_stem_id))).arg(
                       bitset_to_string(grammar_stem_sources));
        } else {
        ...
        if (type == STEM && grammar_stem_id != -1) {
            if (resolve_conflict("stem", "grammar_stem_id", grammar_stem_id, QString("id = %1").arg(item_id), source_id,
                                 true) < 0) {
                _error << "Could not resolve conflict\n";
                return -1;
            }
        }
    }
    ...
    if (existsEntry(item_category, -1, primary_condition, false)) {
               ...
               INVALID_BITSET /*|| abstract_categories!=INVALID_BITSET*//*check this change*/); //assumed to mean row was modified
        {
            if (type == STEM) {
                ...
                if (resolve_conflict(item_category, "reverse_description", QVariant((bool)lemma_ID.toInt()), primary_condition,
                                     source_id, false) < 0) {
                    _error << "Could not resolve conflict\n";
                    return -1;
                }
    ...
    if (type == STEM) {
        ...
        stmt = stmt.arg(item_id).arg(category_id).arg(bitset_to_string(abstract_categories)).arg(bitset_to_string(sources)).arg(
                   raw_data).arg(description_id).arg(POS).arg(lemma_ID);
    } else {
        ...
        stmt = stmt.arg(table).arg(item_id).arg(category_id).arg(bitset_to_string(abstract_categories)).arg(bitset_to_string(
                    sources)).arg(raw_data).arg(description_id).arg(POS).arg(lemma_ID.toInt());
    }
...
int remove_item(item_types type, long item_id, QString raw_data, long category_id, long description_id, QString POS) {
    ...
    if (theSarf->query.numRowsAffected() == 0) {
        ...
        if (theSarf->query.numRowsAffected() == 0) {
            _error << "No entry removed in " + interpret_type(type) + "_category!\n";
            return -2;
        }
    } else if (theSarf->query.numRowsAffected() > 1) {
        _error << "More than one entry in " + interpret_type(type) + "_category removed!\n";
        return -2;
    }
...
long display_table(QString
                   table) { //TODO: has some error in producing sources for example may result in "3,0,0" and also in rules type may result in "AA" always
    theSarf->out << "--------------------------------------------------\n" << table << ":\n";
    ...
    while (theSarf->query.next()) {
        ...
        for (int i = 0; i < columns; i++) {
            if (i == source_column) {
                ...
                theSarf->out << QString("%1\t").arg(!row.value(i).isNull() ? (!(row.value(i).toString() == QString("")) ? row.value(
                                                        i).toString() : QString("    ")) : QString("NULL"));
            }
        }
...
int insert_source(QString name, QString normalization_process, QString creator) { //returns current number of sources
    ...
    stmt = QString("INSERT INTO source(description, normalization_process, creator, date_start,date_last) VALUES(\"%1\", \"%2\", \"%3\", NOW(), NOW())").arg(
               name, normalization_process, creator);
    perform_query(stmt);
...
int insert_compatibility_rules(rules rule, long id1, long id2, long result_id, QString inflectionRule, int source_id) {
    ...
    stmt = QString("SELECT resulting_category FROM compatibility_rules WHERE category_id1=%1 AND category_id2=%2 AND type=%3").arg(
               id1).arg(id2).arg((int)rule);
    perform_query(stmt);
    ...
    if (theSarf->query.size() > 0) { //already present
        while (theSarf->query.next()) {
            ...
            if (old_result_id == result_id) {
                addSource("compatibility_rules", source_id, -1, QString("category_id1=%1 AND category_id2=%2").arg(id1).arg(id2),
                          false);
                insert_rule = false;
                break;
            } else {
                if (KEEP_OLD) {
                    ...
                    sources.setBit(bit_index);
                    stmt = QString("UPDATE compatibility_rules SET resulting_category='%1' ,sources='%2' WHERE category_id1 = '%3' AND category_id2 = '%4'").arg(
                               result_id).arg(bitset_to_string(sources)).arg(id1).arg(id2);
                    //qDebug() << stmt;
                    perform_query(stmt);
...
insert:
        ...
        stmt = stmt.arg(id1).arg(id2).arg((int)(rule)).arg(bitset_to_string(sources)).arg((result_id == -1 ? QString("-1") :
                QString("%1").arg(result_id))).arg(inflectionRule);
        perform_query(stmt);
    }
...
int insert_compatibility_rules(rules rule, QString category1, QString category2, QString category_result,
                               QString inflectionRule, int source_id) {
    ...
    if (id1 == -1) {
        _error << QString("Category 1 Undefined: '%1'\n").arg(category1);
        return -1;
    }

    if (id2 == -1) {
        _error << QString("Category 2 Undefined: '%1'\n").arg(category2);
        return -1;
    }

    if (id_r == -1) {
        _error << QString("Resulting Category Undefined: '%1'\n").arg(category_result);
        return -1;
    }
...
bool areCompatible(rules rule, long category1, long category2, long &resulting_category) {
    QString resulting = getColumn("compatibility_rules", "resulting_category", -1,
                                  QString("category_id1=%1 AND category_id2=%2 AND type =%3").arg(category1).arg(category2).arg((int)rule), false);
    ...
    if (resulting == "") {
        ...
        if (rule == AA || rule == CC) {
            long result_id = resulting.toULongLong(&ok);
